# https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix/blob/9f8f61e5a375c2e01c5187d093ce9c2409f409b0/datasets/download_cyclegan_dataset.sh
FILE=$1
...
if [[ $FILE == "cityscapes" ]]; then
    ...
    echo "You need to download gtFine_trainvaltest.zip and leftImg8bit_trainvaltest.zip. For further instruction, please read ./dataset/prepare_cityscapes_dataset.py"
    exit 1
fi

echo "Specified [$FILE]"
URL=http://efrosgans.eecs.berkeley.edu/cyclegan/dataset/$FILE.zip
ZIP_FILE=./data/dataset/cyclegan/$FILE.zip
TARGET_DIR=./data/dataset/cyclegan/$FILE/
wget -N $URL -O $ZIP_FILE
mkdir $TARGET_DIR
unzip $ZIP_FILE -d ./data/dataset/cyclegan
rm $ZIP_FILE
